setwd()
source("MAP.R")
EM = function (X,Y,Z,mu,sigma,k,em_iter,map_iter,epsilon_em, epsilon_map) {
  m = nrow(Y)
  n = ncol(Y)
  y = vec(Y)
  ...
  for (t in 1:em_iter){
    ...
    map = MAP(X,Y,Z,mu,sigma,k,map_iter,sp = 0, epsilon_map = 0.001)
    X = map$X
   #matrix(as.numeric(map$X),nrow = 600, ncol = 338)
    ...
    for (l in 1:k){
      ...
      #term1 = 1/sqrt(2*pi*sigma[l]^2)*exp(-(y-mu[l])%*%t((y-mu[l]))/(2*sigma[l]^2))
      term2 = term1%*%0
      #term2 = matrix(NA, nrow = m*n, ncol = 1)
      for (index in 1:(m*n)) {
       i = ij(index,m)[1]
       j = ij(index,m)[2]
    ...
    # if ((t>=3) && (sd(sum_U[(t-2):t])/sum_U[t] < 0.001)) {
